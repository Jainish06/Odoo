...
class SaleOrder(models.Model):
    ...
    def button_get_docs(self):
        ...
        for line in self.order_line:
            ...
            for doc in docs:
                ...
                if record:
                    self.env['document.line'].write({'product_ids' : [(4,line.product_template_id.id)]})
                else:
                    self.env['document.line'].create({
                        'doc_id' : doc.id,
                        'product_ids' : [(4,line.product_template_id.id)],
    ...
    def button_update_docs(self):
        body = ('SO of %s updated.', self.partner_id.name)
